...
export default function EmotionRegulator({ childId, addStars, giveReward }) {
  ...
  async function pickStrategy(strat) {
    addStars(10);
    speak(strat.l);
    try { await api.post('/emotion', { emotion: emotion.id, intensity, strategyChosen: strat.l }); } catch {}
    giveReward({
      monster: '🌟',
      title: '¡Muy bien!',
...
const s = {
  ...
  stepper: {
    ...
    borderBottom:'2px solid #F0ECE4', flexShrink:0,
  },
  stepItem:  { display:'flex', alignItems:'center', gap:'8px' },
  stepDot: {
    width:'28px', height:'28px', borderRadius:'50%',
    display:'flex', alignItems:'center', justifyContent:'center',
    fontSize:'13px', fontWeight:900, transition:'all .3s',
  },
  stepLabel: { fontSize:'14px', fontWeight:800, transition:'color .3s' },
  content: {
    flex:1, display:'flex', flexDirection:'column', alignItems:'center',
    padding:'24px 20px', gap:'20px', overflowY:'auto',
  },
  ...
  emCard: {
    ...
    padding:'20px 10px', borderRadius:'24px', cursor:'pointer',
    transition:'transform .15s', boxShadow:'0 3px 14px rgba(0,0,0,.08)',
  ...
  stratCard: {
    ...
    padding:'18px 22px', borderRadius:'22px', cursor:'pointer',
    boxShadow:'0 3px 14px rgba(0,0,0,.08)', transition:'transform .15s',
    fontFamily:"'Nunito', sans-serif",
  },
  ...
  skipBtn: {
    ...
    color:'#8A8780', fontSize:'14px', fontWeight:700, cursor:'pointer',
  },
};
